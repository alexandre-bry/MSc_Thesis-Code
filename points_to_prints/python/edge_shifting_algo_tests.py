...
class Criterion(ABC):
    ...
    @abstractmethod
    def evaluate_segments(self, segments: List[Segment]) -> float:
        raise NotImplementedError("Subclasses must implement this method")


class LinearCriterion(Criterion):
    alpha = 0.1
    use_perimeter_difference = True
    ...
    def evaluate_segments(self, segments: List[Segment]) -> float:
        ...
        current_perimeter = sum(segment.length() for segment in segments)
        if self.use_perimeter_difference:
            if current_perimeter > self.initial_perimeter:
                perimeter_value = self.alpha * abs(
                    current_perimeter - self.initial_perimeter
                )
            else:
                perimeter_value = self.alpha * abs(
                    self.initial_perimeter - current_perimeter
                )
        else:
            perimeter_value = self.alpha * current_perimeter

        return proximity_value + perimeter_value
...
class EdgeShiftingAlgorithm:
    ...
    def evaluate_lines(self, idxs: list[int], moved_lines: dict[int, Line]) -> float:
        ...
        criterion = self.criterion.evaluate_segments(segments)
