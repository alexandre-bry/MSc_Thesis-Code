...
def lod22_to_roof_implementation(
    lod22_path: Path,
    roof_path: Path,
    overwrite: bool,
    skip_existing: bool,
    ...
    """
    Extracts the roof from a LOD2.2 CityJSON file and saves it as a separate CityJSON file.

    Parameters
    ----------
    lod22_path : Path
        Path to the input LOD2.2 CityJSON file.
    roof_path : Path
        Path where the resulting roof-only CityJSON file will be saved.
    overwrite : bool
        Whether to overwrite the output file if it already exists.
    skip_existing : bool
        Whether to skip processing if the output file already exists.
