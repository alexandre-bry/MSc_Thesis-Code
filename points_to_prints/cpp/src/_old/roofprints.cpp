...
//     // For each outline, find the points that are within its bounding box
//     plus a
//     // buffer
...
// //             double dist = std::sqrt(CGAL::squared_distance(segment,
// point));
...
// //           distance_threshold(distance_threshold), dis(0, points.size() -
// 1),
...
//     // // Identify the points that are close to the new edge defined by the
//     best
...
//     //     // Not enough points to fit a line, return the original line with
//     the
//     //     // best offset
...
//     // Segment_2 fitted_segment =
//     Segment_2(fitted_line.projection(new_start),
...
// //     Vector_2 start_to_end = base_end - base_start;
// //     double start_to_end_length =
// CGAL::sqrt(start_to_end.squared_length());
// //     Vector_2 start_to_end_unit = start_to_end / start_to_end_length;
...
// /**
//  * @brief Get the angle in degrees between two edges defined by their start
//  and
...
//             CustomCGAL::Angle angle = get_angle_between(edge_prev,
//             edge_next); if (angle.in_degrees() < ANGLE_THRESHOLD_DEGREES) {
...
//                 double edge_new_length =
//                 std::sqrt(edge_new.squared_length()); Line_2
//                 line_new(point_prev, point_next); Point_2 projection =
//                 line_new.projection(point); double distance_to_projection =
...
//         } else {
//             throw std::runtime_error("Unexpected value for IsGenerated
//             field");
//         }
...
//             CustomCGAL::Angle angle = get_angle_between(prev_edge,
//             curr_edge); if (angle.in_degrees() < ANGLE_THRESHOLD_DEGREES) {
//                 // Check if the projection of the common point of the two
...
//                     std::sqrt(CGAL::squared_distance(projection,
//                     common_point));
...
//             Segment_2 new_segment(other_point_1, other_point_2);
//             double new_segment_length =
//             std::sqrt(new_segment.squared_length()); Line_2
//             new_line(other_point_1, other_point_2); Point_2 projection =
//             new_line.projection(common_point); double distance_to_projection
//             =
...
//         edge_lengths[i] =
//             std::sqrt(CGAL::squared_distance(edges[i].first,
//             edges[i].second));
//     }
...
//                                   offset_dir, MAX_ABSOLUTE_OFFSET,
//                                   OFFSET_STEP);
...
//             roofprint_points.push_back(line2.point(0)); // start point of
//             line2
//         }
//     }
...
//     std::cout << "Selecting points for each selected outline..." <<
//     std::endl; std::vector<OutlineWithPoints> outlines_with_points;
//     select_points_per_outlines(las_reader, selected_outlines,
//                                outline_buffer_distance,
//                                outlines_with_points);
...
//             compute_roofprint_segments(outline_with_points,
//             las_reader.points);
