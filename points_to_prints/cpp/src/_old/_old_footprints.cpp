...
//  * In its local space, the X axis is along the segment, the Y axis is
//  vertical,
...
//         // Binary search to find the interval in limits_intervals that
//         contains
//         // x
//         std::size_t left = 0;
...
//                     "Limits should be ordered in the direction of the
//                     segment");
//             }
...
//     Bbox_2 area_of_interest = outside_boundary.bbox() +
//     inside_boundary.bbox();
...
//     kd_tree->search_indices_in_box(area_of_interest, 0.0,
...
//     footprint_edge = Segment_2(start_2d + best_translation *
//     direction_inwards,
//                                end_2d + best_translation *
//                                direction_inwards);
// }
...
//     if (!status.ok()) {
//         std::cerr << "Error reading edges Parquet file: " <<
//         status.ToString()
//                   << std::endl;
//         return status;
//     }
...
//         std::string cleabs = roofprints_output.value<std::string>("cleabs",
//         i); std::string origine_du_batiment =
...
//     std::cout << "Processing roofprints to compute footprints..." <<
//     std::endl; std::vector<MultiLineStringZWithAttributes> footprints;
//     footprints.reserve(roofprints.size());
//     ProgressBarTotal progress_bar(roofprints.size(), "Processing
//     roofprints"); progress_bar.start(); for (const auto &roofprint :
//     roofprints) {
...
//                 PtsStructs::PointId
//                 new_idx(las_writer.points->point_count()); for (const auto
//                 &dim : initial_pdal_dims) {
...
//     status = write_geoms_to_parquet(footprints, output_footprints_file,
//     true); if (!status.ok()) {
